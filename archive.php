	...
	<div class="mlt-container">
		<header class="archive-header">
			...
			if (is_category()) {
				...
				echo '<h1 class="archive-title">' . esc_html__('Tag: ', 'dental-care') . single_tag_title('', false) . '</h1>';
				echo tag_description();
			} elseif (is_author()) {
				$author_id = get_the_author_meta('ID');
				echo '<h1 class="archive-title">' . esc_html__('Author: ', 'dental-care') . get_the_author() . '</h1>';
				echo '<div class="author-bio">' . get_the_author_meta('description') . '</div>';
			} elseif (is_date()) {
				if (is_day()) {
					echo '<h1 class="archive-title">' . esc_html__('Archives: ', 'dental-care') . get_the_date() . '</h1>';
				} elseif (is_month()) {
					echo '<h1 class="archive-title">' . esc_html__('Archives: ', 'dental-care') . get_the_date('F Y') . '</h1>';
				} elseif (is_year()) {
					echo '<h1 class="archive-title">' . esc_html__('Archives: ', 'dental-care') . get_the_date('Y') . '</h1>';
				}
			} elseif (is_search()) {
				echo '<h1 class="archive-title">' . sprintf(esc_html__('Search Results for: %s', 'dental-care'), '<span>' . get_search_query() . '</span>') . '</h1>';
			} else {
		...
		if (have_posts()) {
			...
			<article class="no-posts">
				...
				<p><?php esc_html_e('No posts found in this archive.', 'dental-care'); ?></p>
				<?php get_search_form(); ?>
			</article>
			<?php
		}
